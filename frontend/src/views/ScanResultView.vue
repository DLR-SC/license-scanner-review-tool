          ...
          >
            ...
            <template v-else-if="currentFinding">
              ...
              <AppCard>
                ...
                ><button v-if="(fileContent[0]?.number ?? 0) > 1" type="button" class="w-full flex items-center gap-2 px-3 py-px bg-blue-50 hover:bg-blue-100 select-none text-blue-600 cursor-pointer" @click="expandAbove"><span class="text-gray-500 mr-4 pr-3 border-r border-gray-200 inline-block w-8 text-right">···</span><span>↑ Load 10 more lines</span></button><template v-for="line in fileContent" :key="line.number"><div :class="line.highlighted ? 'bg-yellow-100' : ''" class="px-3 py-px"><span class="select-none text-gray-500 mr-4 pr-3 border-r border-gray-200 inline-block w-8 text-right">{{ line.number }}</span>{{ line.content }}</div></template><button v-if="(fileContent.at(-1)?.number ?? 0) < fileTotalLines" type="button" class="w-full flex items-center gap-2 px-3 py-px bg-blue-50 hover:bg-blue-100 select-none text-blue-600 cursor-pointer" @click="expandBelow"><span class="text-gray-500 mr-4 pr-3 border-r border-gray-200 inline-block w-8 text-right">···</span><span>↓ Load 10 more lines</span></button></pre>
                <div
