...
test('exclude form submit: PUT body correct, form closes', async ({ page }) => {
  const returnedExclude = { pattern: 'tests/**', reason: 'TEST_TOOL_OF', comment: '' }
  ...
  expect(body.reason).toBe('TEST_TOOL_OF')
...
test('remove path exclude: DELETE sent with correct params', async ({ page }) => {
  const existing = { pattern: 'tests/**', reason: 'TEST_TOOL_OF', comment: '' }
